...
echo "${NEON_GREEN}${BOLD}        INICIANDO EXECUÇÃO      ${RESET}"
echo "${ROYAL_BLUE}${BOLD}================================${RESET}"
echo
...
echo 'Page served from: $region1 - Carolalx' > /var/www/html/index.html
...
apt-get update -y
apt-get install apache2 -y
echo 'Page served from: $region2 - Carolalx' > /var/www/html/index.html
systemctl restart apache2" >/dev/null 2>&1 &
...
echo
echo "${ROYAL_BLUE}${BOLD}==========================${RESET}"
echo "${NEON_GREEN}${BOLD}   LABORATÓRIO CONCLUÍDO  ${RESET}"
echo "${ROYAL_BLUE}${BOLD}==========================${RESET}"
echo
